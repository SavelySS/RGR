...
class client {
 ...
 public:
  ...
  void Print() {
    for (int i = 0; i < n; i++) {
      cout << "----------------------------------------------------" << endl;
      cout << "Email/ИНН (что-то уникальное): " << setw(11) << (hu + i)->email
           << endl;
...
class Zakazes : public client {
 private:
  zakazi zak[20] = {{"cc@gmail.com", 1, 2, {{2, 1}, {1, 3}}},
 ...
 public:
  ...
  void Print() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      cout << "Количество различных товаров:" << setw(5) << (zak + i)->colTov
           << endl;
  ...
  void add() {
    ...
    if (status) {
      ...
      cout << "Заказчика с таким email нет" << endl;
    }
  }
